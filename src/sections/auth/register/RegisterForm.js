...
export default function RegisterForm() {
    ...
    const methods = useForm({
        resolver: yupResolver(RegisterSchema), defaultValues,
    });
    ...
    const onSubmit = async (data) => {
        ...
        try {
            await register( data.firstName, data.lastName, data.year, data.branch, data.div, data.rollNumber, data.moodleId,data.email, data.password);
        } catch (error) {
            console.error(error);
            reset();
        ...
        <Stack spacing={2}>
            ...
            <Stack direction={{xs: 'column', sm: 'row'}} spacing={2}>
                ...
                <RHFSelect name="year" label="Year" placeholder="Year">
                    <option value=""/>
                    {["FE", "SE", "TE", "BE"].map((option, index) => (<option key={index} value={option}>
                        {option}
                    </option>))}
                </RHFSelect>
