...
export default function LoginForm() {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            await login(data.moodleId, data.password);
        } catch (error) {
            console.error(error);
            reset()
    ...
    return (<FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
        <Stack spacing={3}>
            {!!errors.InvalidCredentials && <Alert severity="error">{errors.InvalidCredentials?.message || "Internal server error. Please do not autofill form."}</Alert>}
